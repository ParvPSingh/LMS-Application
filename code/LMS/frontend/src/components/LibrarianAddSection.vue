    ...
    <div class="col-md-4">
        <form id="list_form" class="custom-form">
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="listTitle" placeholder="Section Name" v-model="section_info.name">
            </div>
            <div class="mb-3">
                <label for="description" class="form-label">Section Description</label>
                <textarea class="form-control" name="listDescription" rows="3" maxlength="50" v-model="section_info.description"></textarea>
            </div>
            <button type="submit" v-on:click.prevent="add_section" class="btn btn-primary">Submit</button>
        </form>
    </div>
    <div class="col-md-4"></div>
</div>
    ...
    export default{
        ...
        methods:{
            async add_section(){
                ...
                try {
                const response = await axios.post("http://127.0.0.1:5000/api/sec",
                {
                sec_name: this.section_info.name,
                sec_description: this.section_info.description,
                sec_user_id: this.section_info.sec_user_id
            },
                {
                    headers: {
